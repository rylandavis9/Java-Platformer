...
public class TileManager {
    ...
    public List<KillTile> killTiles = new ArrayList<>();
    ...
    public TileManager(GamePanel gp) {
        this.gp = gp;
        loadTilesFromFolder("res/maps"); // <-- set this path to your exported tiles folder
    ...
    public void buildCollisionTilesFromMap(String[][] map) {
        ...
        killTiles.clear();// Reset if regenerating
        ...
        for (int row = 0; row < map.length; row++) {
            for (int col = 0; col < map[row].length; col++) {
                ...
                } else if (tileCode.equals("k0")) {
                    int worldX = col * tileSize;
                    int worldY = row * tileSize;
                    killTiles.add(new KillTile(worldX, worldY));
                } else  {
                    continue;
                }
            }
